...
class ColorFilter(object):
    ...
    def dominantColors_DBSCAN(self, array:np.array, plot:bool):
        ...
        if self.fmt == 'hsv':
            ...
            colors = np.array([np.average(array[idx==class_],axis = 0) for class_ in range(no_class)],dtype=np.int16)
        else:
...
def extract_color_clustering(imgs:list, mask_results:np.array ,color_fmt:str, plot:bool): #input: BGR img 
    # input img
    color_filter = ColorFilter(color_fmt)
    
    # masked_arrays=[imgs[0].reshape(int(imgs[0].size/3),3)]
    masked_arrays  = color_filter.mask_to_array([imgs[0]],[mask_results[0]])   # return pixel result , [(h*w , c)]
    ...
    json_file = "/py/clothes_seg/config/color.json"

...
# from libc.stdfloat cimport 

cdef extern from "argmax_API.c":
    void Cargmax(void *img_array, void *mask_array, unsigned long N ) nogil
...
cpdef list postprocess_cython(cnp.ndarray masks, list pads, list resized_imgs): # inp:batch
    ...
    cdef cnp.ndarray[cnp.uint8_t, ndim=3] mask_argmax_batch = np.ndarray((N, net_h, net_w), dtype=np.uint8)
    ...
    cdef float[:,:,:,:] mask_batch = mask_batch_
    cdef float* mask_batch_ptr = &mask_batch[0][0][0][0]
    cdef float* mask_batch_ptr_W = &mask_batch[0][0][0][1]
    cdef float* mask_batch_ptr_B0 = &mask_batch[0][0][0][0]
    cdef float* mask_batch_ptr_B1 = &mask_batch[1][0][0][0]
    cdef float* mask_batch_ptr_B2 = &mask_batch[2][0][0][0]

    # cdef float* mask_batch_ptr_W = &mask_batch[0][0][0][1]
    # cdef float* mask_batch_ptr_H = &mask_batch[0][0][1][0]
    # cdef float* mask_batch_ptr_C = &mask_batch[0][1][0][0]
    # cdef float* mask_batch_ptr_N = &mask_batch[1][0][0][0]
    cdef unsigned char* mask_argmax_batch_ptr = &mask_argmax_batch_view[0][0][0]
    cdef vector[int] ignore_class = [0,1,2,3,4,5,9,10,13,14,15,16,17,18] 

    # print("===========================",mask_batch_ptr)
    # printf("ptr: %#010x\n", mask_batch_ptr)
    # printf("ptr: %#010x\n", mask_batch_ptr_W)
    # printf("ptr_H: %#010x\n", mask_batch_ptr_H)
    # printf("ptr_W: %#010x\n", mask_batch_ptr_C)
    ...
    with nogil:
        ...
        Cargmax(mask_batch_ptr, mask_argmax_batch_ptr, N)
